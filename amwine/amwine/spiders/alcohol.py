...
class AlcoholSpider(scrapy.Spider):
    ...
    start_urls = ['https://amwine.ru/catalog/krepkie_napitki/konyak/a-de-fussigny-selection/']
    page_count = 4
    ...
    def start_requests(self):
        ...
        for page in range(1, self.page_count):
            yield scrapy.Request(request_url,
                                 body=f'json=y&params%5BIBLOCK_TYPE%5D=catalog&params%5BIBLOCK_ID%5D=2&params%5BCACHE_TYPE%5D=Y&params%5BCACHE_TIME%5D=3600&params%5BSECTION_ID%5D=18&params%5BSECTION_CODE%5D=konyak&params%5BPRICE_CODE%5D=MOSCOW&params%5BPAGE_ELEMENT_COUNT%5D=18&params%5BFILTER_NAME%5D=arrFilterCatalog&params%5BSORT_ORDER%5D=ASC&params%5BSORT_FIELD%5D=SORT&params%5BMESSAGE_404%5D=&params%5BSET_STATUS_404%5D=&params%5BSHOW_404%5D=Y&params%5BFILE_404%5D=&params%5BNO_INDEX_NO_FOLLOW%5D=N&params%5BCURRENT_PAGE%5D=1&params%5B~IBLOCK_TYPE%5D=catalog&params%5B~IBLOCK_ID%5D=2&params%5B~CACHE_TYPE%5D=Y&params%5B~CACHE_TIME%5D=3600&params%5B~SECTION_ID%5D=18&params%5B~SECTION_CODE%5D=konyak&params%5B~PRICE_CODE%5D=MOSCOW&params%5B~PAGE_ELEMENT_COUNT%5D=18&params%5B~FILTER_NAME%5D=arrFilterCatalog&params%5B~SORT_ORDER%5D=ASC&params%5B~SORT_FIELD%5D=SORT&params%5B~MESSAGE_404%5D=&params%5B~SET_STATUS_404%5D=&params%5B~SHOW_404%5D=Y&params%5B~FILE_404%5D=&params%5B~NO_INDEX_NO_FOLLOW%5D=N&params%5B~CURRENT_PAGE%5D=1&current_filter%5BACTIVE%5D=Y&current_filter%5BIBLOCK_ID%5D=2&current_filter%5BINCLUDE_SUBSECTIONS%5D=Y&current_filter%5BSECTION_ID%5D=18&PAGEN_1={page}&sort=sort',
    ...
    def parse_alco(self, response):
        ...
        for params in response.css('div.about-wine__block div.h4::text').getall():
            discription_right_title.append(params.strip())
        for params1 in response.css('div.about-wine__block p::text').getall():
            discription_right += discription_right_title[i] + " " + params1.strip()
            i += 1
        ...
        item = {
            ...
            'main_image': url_image,  # {str} Ссылка на основное изображение товара
